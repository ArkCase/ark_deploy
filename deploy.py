...
def run(args, notrace: bool=False, input: str=""):
    """
    Run a command and return the output. The output returned by the command
    must be a human-readable string. If the command fails, this function exits
    immediately.
    ...
    try:
        if input:
            output = subprocess.check_output(args, text=True, input=input)
        else:
...
add_helm_repo("arkcase", "https://arkcase.github.io/ark_helm_charts")
...
##run("helm -n observability install -f kiali-values.yaml kiali ../../helm-charts/kiali-server")
...
if cfg['platform'] == "eks":
    info("*** Installing/Updating AmazonMQ ***")
    err("AmazonMQ not yet implemented")
    # TODO
    sys.exit(1)
else:
